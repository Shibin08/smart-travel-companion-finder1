...
export default function ReviewsPage() {
  ...
  useEffect(() => { document.title = 'Reviews — Travel Companion Finder'; }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <p className="text-gray-600">Submit and view reviews synced with backend data.</p>
      </div>

      {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm">{error}</div>}

      {isLoadingMatches ? <ReviewSkeleton /> : <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
        <h2 className="text-lg font-semibold text-gray-900">Select matched traveler</h2>
        {matches.length === 0 ? (
            ...
            {matches.map((match) => (
              <button
                ...
                className={`text-left p-3 rounded-lg border ${
                  selectedMatch?.match_id === match.match_id ? 'border-teal-400 bg-teal-50' : 'border-gray-200 bg-white'
                }`}
              >
      ...
      {selectedMatch && user && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
          <h3 className="text-lg font-semibold text-gray-900">Write review for {selectedMatch.other_user.name}</h3>
          ...
          <button
            ...
            className="px-4 py-2 rounded-md bg-teal-600 text-white text-sm font-medium hover:bg-teal-700 disabled:opacity-50"
          >
      ...
      {selectedMatch && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Reviews for {selectedMatch.other_user.name}</h3>
